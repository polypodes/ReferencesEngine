...
class FakerGenerator
{
    ...
    public function getFake()
    {
        //Set services as var
           ...
           $em           = $this->em;
        //check if dir exist
           $dir = $kernel->getRootDir().'/../web/uploads/media/tmp/';
        //route to save the fake binary content
        if (!file_exists($dir)) {
        ...
        for ($i = 0; $i < $nbMediaGenerated; $i++) {
            ...
            // get BinaryContent, $image is the url of the file downloaded
            //save Media
        ...
        for ($i = 0; $i < $nbFicheGenerated; $i++) {
            ...
            $fiche->getFake();
            //get a faked Fiche
            $data_uniqueTagsArray = $this->getRandomUniqueInt('0', ($nbTagGenerated - 1), $nbMinTagByFicheSet, $nbMaxTagByFicheSet);
            //get unique array of int for tags
            foreach ($data_uniqueTagsArray as $uniqueTag => $value) {
                   $fiche->addTag($datatag[$value]);
                //set tags
            }
            ...
            foreach ($data_uniqueRendersArray as $uniqueRender => $value) {
                   $fiche->addRender($datamedia[$value]);
                //set renders
            }
            ...
            foreach ($data_uniqueMediasArray as $uniqueMedia => $value) {
                   $fiche->addMedia($datamedia[$value]);
                //set medias
            }

            $fiche->setImage($datamedia[(rand(0, ($nbMediaGenerated - 1)))]);
            //set main images
            $datafiche[] = $fiche;
            $em->persist($fiche);
            $em->flush();
